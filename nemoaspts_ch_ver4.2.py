...
##csv export_SGH
def csvmake():
    yday = datetime.datetime.now()-datetime.timedelta(minutes=1)
    nowtime = yday.strftime('%H:%M')
    print(nowtime)
    csvsql = "select *  from nemosys where date_format(time,'%H:%i') ='"+nowtime+"' into outfile '/var/lib/mysql-files/"+nowtime+".csv' fields terminated by ',' enclosed by '\"' lines terminated by '\n'"
    cur.execute(csvsql)
    conn.commit()
...
##csv export time_SGH
schedule.every(1).minutes.do(csvmake)

...
def Current():
    ...
    if(max(Current_list)<1.0 and suncalv>118000):
        print("err")
        Dev_Corrent=0
...
schedule.every(1).minutes.do(Current)
try :      
        while 1 : 
            ...
            sql="insert into nemosys(id,time,`sunvoltage(mV)`,`Development(mV)`,pcbsignal) values('system1','"+now.strftime("%Y-%m-%d %H:%M:%S")+"',"+str(suncalv)+","+str(Development)+","+str(pcbsignal)+")"
            cur.execute(sql)
            ...
            #Signal Control
            if(suncalv<300000) : #Result of 2 channel integral has to satisfy under 300V
            ...
            doc_ref1 = db.collection('nemosystem').document('Collected_data')
            doc_ref1.set({
                'id' : "system1", 'time' : utc_now , 'sunvoltage' : suncalv, 'inverter1calv' : Dev_Corrent , '' : inverter2calv ,
                'invertervoltage3' : inverter3calv , 'sunvoltage3' : 0 , 'sunvoltage4' : 0 , 'MPPtime' : Development, 'pcbsignal' : pcbsignal
                })
